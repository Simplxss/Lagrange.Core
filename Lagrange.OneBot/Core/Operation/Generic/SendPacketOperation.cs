...
public class SendPacketOperation : IOperation
{
    public async Task<OneBotResult> HandleOperation(BotContext context, JsonNode? payload)
    {
        if (payload.Deserialize<OneBotSendPacket>() is { } send)
        {
            ...
            var ssoPacket = new SsoPacket(send.PacketType, send.EncodeType, send.Command, context.ContextCollection.Keystore.Session.MsgCookie, sequence, new BinaryPacket(send.Data.UnHex()));
