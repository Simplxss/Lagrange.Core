...
internal class ExchangeEmpService : BaseService<ExchangeEmpEvent>
{
        ...
        out BinaryPacket output, out List<BinaryPacket>? extraPackets)
    {
        switch (input.EventState)
        {
            ...
            case ExchangeEmpEvent.State.RefreshD2:
                {
                    var packet = new ExchangeEmp0x000FV2(keystore, appInfo, device, 0); // Todo: ssoseq
                    exchangeEmp = packet;
                    output = packet.ConstructPacket();
                    break;
                }
    ...
    {
        var payload = BitConverter.GetBytes(input.Length, false).Concat(input.ToArray()).ToArray();
        ...
        if (state == ExchangeEmp.State.Success)
        {
            ...
            keystore.Info = new(tlv11A.Age, tlv11A.Gender, tlv11A.Nickname);
            ...
            {
                var host = tmp.ReadString(Prefix.Uint16 | Prefix.LengthOnly);
                var pskey = tmp.ReadString(Prefix.Uint16 | Prefix.LengthOnly);
